...
export default function Home() {
  ...
  // Analyze Link (YouTube or TikTok)
    ...
    try {
      ...
      if (isTikTok) {
        if (json.success) {
          ...
          alert("TikTok විස්තර ලබාගත නොහැකි විය!");
        }
      } else {
        if (json.status) {
          setVideoInfo({
          ...
          alert("YouTube සබැඳිය පරීක්ෂා කරන්න!");
        }
      }
    } catch (error) {
      console.error(error);
  ...
  // YouTube Download (for MP3/Video qualities)
    ...
    <>
      ...
      <div className="main-wrapper">
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .sub-title { color: #a29bfe; font-size: 11px; margin-bottom: 25px; letter-spacing: 2px; opacity: 0.8; text-transform: uppercase; }
          .input-glass {
          ...
          .btn-neon {
            ...
            border: none; border-radius: 12px; color: white; font-weight: 600; cursor: pointer; transition: 0.3s; text-transform: uppercase;
          }
          ...
          .video-title-text { color: #eee; font-size: 13px; margin-bottom: 15px; text-align: left; line-height: 1.4; }
          .video-title-text span { color: #bd93f9; font-weight: 600; }
          .q-label { color: #a29bfe; font-size: 12px; font-weight: bold; text-align: left; margin: 10px 0 5px 2px; text-transform: uppercase; }
          .q-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 15px; }
          .q-btn {
            padding: 10px; background: rgba(138, 43, 226, 0.15); border: 1px solid rgba(138, 43, 226, 0.4);
            color: #fff; border-radius: 10px; font-size: 12px; cursor: pointer; transition: 0.2s; text-decoration: none; display: flex; align-items: center; justify-content: center;
          }
          ...
          .download-link { color: #2ed573; text-decoration: none; font-weight: 600; font-size: 14px; text-transform: uppercase; }
          .back-btn { background: none; border: none; color: #ff4757; marginTop: 20px; cursor: pointer; fontSize: 12px; opacity: 0.8; }
          .back-btn:hover { opacity: 1; }
        `}} />

        <div className="glass-card">
          <h1 className="vortex-title">VORTEX</h1>
          <p className="sub-name" style={{color: '#bd93f9', fontSize: '10px', marginTop: '-5px', marginBottom: '20px', letterSpacing: '3px'}}>ADVANCED DOWNLOADER</p>

          {!videoInfo ? (
            <>
              <input
                className="input-glass"
                placeholder="Paste Link (YouTube or TikTok)..."
                value={url}
            ...
            <div className="result-area">
              ...
              {videoInfo.isTikTok ? (
                <>
                  <div className="q-label">TikTok Downloads:</div>
                  <div className="q-grid">
                    {videoInfo.downloads.map((dl, index) => (
                      <a key={index} href={dl.url} target="_blank" rel="noreferrer" className="q-btn">
                        {dl.type}
                      </a>
                    ))}
                  </div>
                ...
                <>
                  <div className="q-label">Select Format:</div>
                  <div className="q-grid">
                    ...
                    <button className="q-btn" onClick={() => startDownload('1080')}>1080P Full HD</button>
                  </div>
                </>
              )}

              {loading && <p style={{color: '#a29bfe', fontSize: '12px', margin: '10px 0'}}>Processing your request...</p>}

              {downloadLink && (
                <div className="dl-container">
                  <a className="download-link" href={downloadLink.link} target="_blank" rel="noreferrer">
                    ⬇️ DOWNLOAD {downloadLink.format.toUpperCase()}
                  </a>
                </div>
              )}
